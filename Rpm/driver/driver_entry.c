...
NTSTATUS IOCTL(PDEVICE_OBJECT DeviceObject, PIRP irp) {
	...
	case IOCTL_READ_MEM: {
		...
		if (authok && UserBuffer && ValidPointer(buffer->Addr)  && buffer->PID != NULL) {
	...
	case IOCTL_Imgui: {
		...
		p_Imgui buffer = (p_Imgui)irp->AssociatedIrp.SystemBuffer;
		if (buffer->PID != NULL)
		{
			DWORD Key2 = 0xd9426256;
			DWORD Value = 0x3f63c427;
			ULONG dec = buffer->name ^ Key2;
			if (dec == Value)
				authok = TRUE;
			else
				authok = FALSE;
